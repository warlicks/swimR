...
test_events <- c("50 Freestyle", "100 Freestyle") # U
...
meet_data2 <- meet_data %>%
	...
	select(full_name_computed, team_short_name, team_code, swim_time, swim_time2, team_event_rank2)
...
event_score <- meet_data2 %>%
	group_by(team_code) %>%
	mutate(event_score = sum(points)) %>%
	distinct(team_code) %>%
	select(team_code, event_score)
